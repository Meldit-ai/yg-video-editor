...
describe("ReelTarget", () => {
  ...
  it("orders by Instagram post time, unknown last, then by import time", async () => {
    reel.findMany.mockResolvedValueOnce([
      { id: "r1", mediaUrl: "https://ig/1.mp4", postedAt: T0, createdAt: T1 },
      { id: "r2", mediaUrl: "https://ig/2.mp4", postedAt: null, createdAt: T0 },
    ]);

    await expect(target.loadPending("cmp_1")).resolves.toEqual([
      { id: "r1", url: "https://ig/1.mp4", arrivedAt: T0 },
      // No post time: the import time stands in, and it sorts after anyway.
      { id: "r2", url: "https://ig/2.mp4", arrivedAt: T0 },
    ]);
